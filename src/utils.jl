...
check_constant_cols(X::Matrix{T}) where {T<:AbstractFloat} = size(X,1)==1 || (i=find(all(X .== X[1,:]',1))) == [] || error("You must remove constant columns $i of input data (X) before train")
